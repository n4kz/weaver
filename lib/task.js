...
function spawn (id) {
	...
	/* TODO: Allow other executables, not only nodejs */
	subtask.process = fork(process.execPath, [this.source].concat(subtask.args), {
		stdio : 'pipe',
	...
	subtask.process.stdout.on('data', writer.bind(process.stdout, process.stdout.write, subtask.pid + ' (' + subtask.name + ') '));
	subtask.process.stderr.on('data', writer.bind(process.stderr, process.stdout.write, subtask.pid + ' [' + subtask.name + '] '));
...
function writer (write, prefix, data, encoding) {
	this.write(prefix + data.toString(), encoding);
}
