...
function Task (name, options) {
	...
	if (!(name in tasks)) {
		...
		/**
		 * Minimal run time required for persistent task restart
		 * after crash
		 * @type Number
		 * @default 1000
		 */
...
function onExit (task, code, signal) {
	...
	if (this.persistent && code && Date.now() - task.time < this.runtime) {
		Anubseran.log('Restart skipped for task id ' + task.id + ' (' + task.name + ')');
		return;
	}
